...
// tab-pane jQuery UI slide transition effect
 ...
 */
  $('.nav-link').click(function (event) {
    ...
    console.log('----------------------------------------');
    console.log(event);
    console.log(event.target);                    // <a class="nav-item nav-link btn-info" id="nav-slice-tab" (targeted button link)
    console.log(event.target.hash);               // target #id
    console.log(event.target.hash+'.tab-pane');   // target #id.tab-pane
    console.log($('.tab-pane.active'));           // current tab: div#nav-admin.tab-pane
    // THIS WORKS without event.stopPropagation(); AND with .tab-pane {position: absolute;}
    $('.tab-pane.active').hide('slide', {direction: 'left'}, 600);    // OK div #nav-admin.tab-pane
    $(event.target.hash+'.tab-pane').stop().show('slide', {direction: 'right'}, 600);   // OK but slides under the previous
    
    console.log($(event.relatedTarget));    // previous
    console.log($(event.target));    // active
  });

  $('.tab-pane.active').show('slide', {direction: 'right'}, 600);   // loads first tab
 });
