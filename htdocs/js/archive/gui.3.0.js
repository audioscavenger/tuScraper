...
3.1: 
- [ ] use of data-semester="id" to generate dynamic urls when "title" is present in table th
- [ ] rewrite tableCreate somehow, because it currently can format only one column
...
// - classes[ClassNumber] if present in the fileds retreived
// - only if title is present in the th
function createClassUrl(semester, ClassNumber) {
  return 'https://mytumobile.towson.edu/app/catalog/classsection/TOWSN/'+semester+'/'+ClassNumber;
}
...
var tableCreate = function () {
  ...
  return function (columns, values, fieldToFormat='timestamp', functionToFormat='formatTimestamp') {
...
$(document).ready(function() {
  ...
  getJsonAsyncToVar("js/tableFormat.dict.json", 'tableFormatDict');
  setNavLinkEvents();
